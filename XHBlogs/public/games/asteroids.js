...
function Asteroids() {
  ...
  this.update = function () {
    ...
    if (this.dying.length) {
      // Deduplicate in case same element was hit by multiple bullets
      const dyingSet = new Set(this.dying);
      for (const el of dyingSet) {
        try {
          // Hide instead of removeChild to avoid conflicts with React-managed DOM
  ...
  function destroy() {
    ...
    if (this.gameContainer?.parentNode) this.gameContainer.parentNode.removeChild(this.gameContainer);
  }
}
